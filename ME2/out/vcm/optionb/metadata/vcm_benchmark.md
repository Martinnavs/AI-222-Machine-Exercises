...
Window: 1.5s (151 log-mel frames). onnxruntime threads: intra_op=1, inter_op=1. Checkpoint: `out/vcm/optionb/checkpoints/checkpoint.pt` (preset=default).

| variant | size (MB) | p50 latency (ms) | p95 latency (ms) | process peak RSS (MB) | inference-only RSS delta (KB) | note |
|---|---|---|---|---|---|---|
| fp32 ONNX | 0.973 | 1.059 | 1.069 | 443.508 | 0 | AMD EPYC 7742 (256-thread node) estimate |
| INT8 ONNX (static, val-calibrated) | 0.264 | 1.032 | 1.042 | 520.094 | 0 | AMD EPYC 7742 (256-thread node) estimate |
