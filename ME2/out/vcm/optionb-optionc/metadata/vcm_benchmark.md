...
Window: 1.5s (151 log-mel frames). onnxruntime threads: intra_op=1, inter_op=1. Checkpoint: `out/vcm/optionb-optionc/checkpoints/checkpoint.pt` (preset=optionc).

| variant | size (MB) | p50 latency (ms) | p95 latency (ms) | process peak RSS (MB) | inference-only RSS delta (KB) | note |
|---|---|---|---|---|---|---|
| fp32 ONNX | 2.877 | 2.820 | 2.850 | 441.934 | 0 | AMD EPYC 7742 (256-thread node) estimate |
| INT8 ONNX (static, val-calibrated) | 0.746 | 2.717 | 2.754 | 539.934 | 0 | AMD EPYC 7742 (256-thread node) estimate |
