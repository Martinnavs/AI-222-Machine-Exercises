...
Window: 1.5s (151 log-mel frames). onnxruntime threads: intra_op=1, inter_op=1. Checkpoint: `out/vcm/option-d-fil50/checkpoints/checkpoint.pt` (preset=optiond).

| variant | size (MB) | p50 latency (ms) | p95 latency (ms) | process peak RSS (MB) | inference-only RSS delta (KB) | note |
|---|---|---|---|---|---|---|
| fp32 ONNX | 3.853 | 3.798 | 3.912 | 488.934 | 0 | AMD EPYC 7742 (256-thread node) estimate |
| INT8 ONNX (static, val-calibrated) | 0.994 | 3.688 | 3.711 | 553.383 | 0 | AMD EPYC 7742 (256-thread node) estimate |
